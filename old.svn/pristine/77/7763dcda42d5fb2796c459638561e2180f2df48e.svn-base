  ...
  async ngOnInit() {
    ...
    switch (action.path) {
      ...
      case LogoutActions.LoggedOut:
            this.message.next('ログアウトしました');
        break;
      default:
        throw new Error(`不正なURLです '${action}'`);
    }
  }
    ...
    if (isauthenticated) {
      ...
      switch (result.status) {
        ...
        default:
          ...
          throw new Error('不正な認証結果です');
      }
    } else {
      this.message.next('正常にログアウトしました');
    }
  }
  ...
  private getReturnUrl(state?: INavigationState): string {
    ...
    if (fromQuery &&
      ...
      // This is an extra check to prevent open redirects.
        throw new Error('戻りURLが不正です。戻りURLは同じドメインの必要があります');
